 ...
 *  ZebraDatePicker
 *
 *  ZebraDatePicker is a small, compact and highly configurable date picker plugin for jQuery
 *
 ...
 *  @package    ZebraDatePicker
 */
;(function($) {

    'use strict';

    $.ZebraDatePicker = function(element, options) {
        ...
        var defaults = {
            ...
            //  if set as a jQuery element with a ZebraDatePicker attached, that particular date picker will use the
        ...
        var init = function(update) {
            ...
            if (!plugin.settings.always_visible) {
                ...
                if (!update) {
                    ...
                    if (plugin.settings.show_icon) {
                        ...
                        var icon_wrapper = $('<span class="ZebraDatePicker_Icon_Wrapper"></span>').css({
                        ...
                        icon = $('<button type="button" class="ZebraDatePicker_Icon' + ($element.attr('disabled') == 'disabled' ? ' ZebraDatePicker_Icon_Disabled' : '') + '">Pick a date</button>');
                ...
                if (undefined !== icon) {
                    ...
                    if (plugin.settings.inside) icon.addClass('ZebraDatePicker_Icon_Inside');
            ...
            $(window).bind('resize.ZebraDatePicker', function() {
            ...
            if (!plugin.settings.always_visible)
                ...
                $(document).bind({
                    ...
                    'mousedown.ZebraDatePicker': function(e) {
                        ...
                        if (datepicker.css('display') == 'block') {
                            ...
                            // hide the date picker
                            if ($(e.target).parents().filter('.ZebraDatePicker').length === 0) plugin.hide();
                    ...
                    'keyup.ZebraDatePicker': function(e) {
        ...
        plugin.destroy = function() {
            ...
            $(document).unbind('keyup.ZebraDatePicker');
            $(document).unbind('mousedown.ZebraDatePicker');
            $(window).unbind('resize.ZebraDatePicker');

            // remove association with the element
            $element.removeData('ZebraDatePicker');
        ...
        var update_dependent = function(date) {
            ...
            if (plugin.settings.pair) {
                ...
                $.each(plugin.settings.pair, function() {
                    ...
                    // chances are that in the beginning the pair element doesn't have the ZebraDatePicker attached to it yet
                    // (as the "start" element is usually created before the "end" element)
                    // so we'll have to rely on "data" to send the starting date to the pair element

                    // therefore, if ZebraDatePicker is not yet attached
                    if (!($pair.data && $pair.data('ZebraDatePicker')))

                        // set the starting date like this
                        $pair.data('zdp_reference_date', date);

                    // if ZebraDatePicker is attached to the pair element
                    else {

                        // reference the date picker object attached to the other element
                        var dp = $pair.data('ZebraDatePicker');
    ...
    $.fn.ZebraDatePicker = function(options) {

        // iterate through all the elements to which we need to attach the date picker to
        return this.each(function() {

            // if element has a date picker already attached
            if (undefined !== $(this).data('ZebraDatePicker'))

                // remove the attached date picker
                $(this).data('ZebraDatePicker').destroy();

            // create an instance of the plugin
            var plugin = new $.ZebraDatePicker(this, options);

            // save a reference to the newly created object
            $(this).data('ZebraDatePicker', plugin);
